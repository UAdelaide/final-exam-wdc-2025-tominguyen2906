...
app.get('/api/walkrequests/open', async (req, res, next) => {
  try {
    const [walkRequests] = await db.execute('SELECT WalkRequests.request_id, Dogs.dog_name, WalkRequests.requested_time, WalkRequests.duration_minutes, WalkRequests.location, owner_id FROM WalkRequests WHERE status = "open"');
